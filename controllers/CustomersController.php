...
class CustomersController extends Controller
{
	...
	public function actionAdd()
	{
      ...
      /*if (($customer->load(Yii::$app->request->post()) && 
           $phone->load(Yii::$app->request->post())) && $this->xstore($customer,$phone)) {
           #($customer->save() &&  $phone->save())) {
            return $this->redirect(['/customers']);
        } else {
				return $this->render('add', compact('customer', 'phone'));
        }*/
   ...
   private function xstore(CustomerRecord $customer_record, PhoneRecord $phone_record)
   {
	...
	private function store(Customer $customer)
   {
	...
	{
		foreach(PhoneRecord::find()->all() as $phone_record){
		  $customer_record = CustomerRecord::findOne($phone_record->customer_id);
		  $records[] = $this->makeCustomer($customer_record, $phone_record);
		}
		$dataProvider = $this->wrapIntoDataProvider($records);
		return $dataProvider;
